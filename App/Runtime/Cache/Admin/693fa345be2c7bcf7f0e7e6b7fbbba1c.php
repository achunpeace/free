                      ...
                      <div class="form-group">
                        ...
                        <?php if(is_array($arr)): foreach($arr as $k=>$val): ?><select class="select" name="pid" style="color:pink;">
                            <!-- 输出顶级分类和杠杠 -->       
                              ...
                              <?php if(is_array($val)): foreach($val as $key=>$v): ?><option <?php if($v['id'] == $k and $v['id'] != $one['id']): ?>selected="true" <?php elseif($v['id'] == $one['id']): ?> disabled<?php endif; ?> value="<?php echo ($v['id']); ?>" >
                                      <?php echo ($v['name']); ?>
