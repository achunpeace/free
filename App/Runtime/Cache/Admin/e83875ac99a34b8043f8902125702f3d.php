                  ...
                  <div class="tile-body color transparent-black rounded-corners">
                    ...
                    <form action="" method="post">
                      <table  class="table table-datatable table-custom" id="basicDataTable" >
                        <thead>
                          <tr id="xlr">
                          <th class="sort-alpha" style="font-size:18px; width:10px; text-align:center; font-weight:900; color:pink;">
                          <input type="submit" value="全部删除"></th>
                          <th class="sort-alpha" style="font-size:18px; width:8px; font-weight:900; color:pink;">排序</th>
                            <th class="sort-alpha" style="font-size:18px; font-weight:900; color:pink;">类名</th>
                            
                            <th class="sort-alpha" style="font-size:18px; text-align:center;">PID</th>
                            <th class="sort-alpha" style="font-size:18px; font-weight:900; color:pink; text-align:center;">操作</th>
                          </tr>
                        </thead>
                        <tbody id="tbody">
                          <!-- 第一个表格 -->
                          <?php if(is_array($menus['child'])): foreach($menus['child'] as $key=>$val): ?><tr class="gradeA" >
                                  <td><input name = "id[]" class="idd" type = "checkbox" value = "<?=$value['id']?>"><?php echo ($val['id']); ?></td>
                                  <td class="order"><input type="text" value="<?php echo ($val['order']); ?>" /></td>
                                  <td class="name"><?php if(!empty($val['child'])): echo ($val['name']); ?><i class="icon-plus plus" lv="0" style="cursor:pointer"></i>
                                   <?php else: ?>
                                   <?php echo ($val['name']); endif; ?></td>
                                  
                                  <td class="pid"><?php echo ($val['pid']); ?></td>
                                  <td class="actions text-center" style="text-align:center;">
                                    <i class="icon-edit"></i>
                                    <a class="edit" href="<?php echo U("edit");?>">编辑</a>
                                    <i class="icon-trash"></i>
                                    <a class="delete" onclick="return confirm('确定要删除吗')" href="<?php echo U(delete);?>}">删除</a>
                                  </td>
              ...
              for(var i in cate){
                  ...
                  str +='<td><input name = "id[]" class="idd" type="checkbox" value = "'+cate[i].id+'">'+cate[i].id+'</td>';
                   ...
                   if(cate[i].child==1){
                    ...
                    str +='</td>';
                    str +='<td class="order">'+cate[i].order+'</td>';
                    str +='<td class="pid">'+cate[i].pid+'</td>';
                    str +='<td class="actions text-center" style="text-align:center;">';
                    str +='<i class="icon-edit"></i>';
                    str +='<a class="edit" href="index.php?admin=1&c=menus&a=edit&id='+cate[i].id+'">编辑</a>';
                    str +='<i class="icon-trash"></i>';
                    str +='<a class="delete" onclick="return confirm('+'"确定要删除吗"'+')" href="index.php?admin=1&c=menus&a=delete&id='+cate[i].id+'">删除</a>';
                     str +='</td>';
                     str +='</tr>';
                      }
                      // alert(str);
